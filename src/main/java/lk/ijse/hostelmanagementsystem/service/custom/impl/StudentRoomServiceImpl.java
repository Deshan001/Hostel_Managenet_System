...
public class StudentRoomServiceImpl implements StudentRoomService {
    ...
    @Override
    public List<ReserveOrAvailableRoomsTM> getReserveRoomDetails() {
        Session session = factory.getSession();
        try {
            List<StudentRoom> reservedRoomsList = studentRoomRepo.getReservedRoomsList(session);
            return converter.convertToReserveRoomTM(reservedRoomsList);
        } catch (Exception e) {
            e.printStackTrace();
        }finally {
            session.close();
    ...
    public HashMap<String, Double> getMonthlyIncome() {
        ...
        try {
            ...
            for(Object[] ob : income){
                for (int i = 0 ; i < ob.length ; i++){
                    System.out.print(ob[i].toString()+" : ");
                }
                System.out.println();
            }
            HashMap<String,Double> stringDoubleHashMap = converter.convertObjectArrayToStringDoubleHashMap(income);
