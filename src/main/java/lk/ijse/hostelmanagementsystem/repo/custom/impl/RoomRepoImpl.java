...
public class RoomRepoImpl implements RoomRepo {
    ...
    @Override
    public List<Room> getAvailableRoom(RoomType roomType,Session session) throws Exception{
    ...
    public List<Room> getAvailableRoom(Session session) throws Exception{
    ...
    public List<Object[]> getCount(Session session) throws Exception{
        Query query = session.createQuery("select distinct count(r.id) as roomCount,r.roomType.id as roomType from Room r" + " where r.availability='available' group by r.roomType.id", Object[].class);
