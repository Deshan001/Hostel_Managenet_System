...
public class StudentRoomRepoImpl implements StudentRoomRepo {
    ...
    @Override
    public List<StudentRoom> getReservedRoomsList(Session session) throws Exception{
        Query query = session.createQuery("from StudentRoom sr where sr.toDate >= :today and sr.room.availability=:av");
    ...
    public List<StudentRoom> getRemainingKeyMoneyList(Session session) throws Exception {
        ...
        Query query = session.createQuery("from StudentRoom  sr where sr.toDate >= :today and sr.advance<sr.room.roomType.key_money");
        LocalDate now = LocalDate.now();
        Date from = Date.from(now.atStartOfDay(ZoneId.systemDefault()).toInstant());
        query.setParameter("today",from);
        List<StudentRoom>list = query.list();
    ...
    public boolean updateKeyMoney(Session session,StudentRoom studentRoom)throws Exception{
        Query query = session.createQuery("update StudentRoom sr set sr.advance=:amount where sr.id=:id");
    ...
    public boolean updateToDate(Session session, StudentRoom studentRoom) throws Exception {
        Query query = session.createQuery("update StudentRoom sr set sr.toDate=:toDate where sr.id=:id");
